...
#if defined (__VMKLNX__)
    /* Fix the DMA mask defaulting to 32-bit (0xffffffff) for USB 2.0 HUBs
    * If USB HUB doesn't support 64-bit addressing the driver will not work
    */
    if (dev->udev->speed == USB_SPEED_HIGH)
	    printk("INFO: Setting DMA MasK\n");
            if (dma_supported (&intf->dev, DMA_64BIT_MASK)) {
                *intf->dev.dma_mask=DMA_64BIT_MASK;
                dev->net->features |= NETIF_F_HIGHDMA;
                }
